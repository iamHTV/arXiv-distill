...
3. **Consistent gains across scales**: Test trên 3 model (4B-30B), 5 benchmarks, luôn cải thiện.
...
## Key Findings
- Qwen3-4B đạt average 59.0 trên 5 benchmarks, improvement +5.7 so với base model, +2.5 so với baseline mạnh nhất (LongRLVR)
...
2. **Process reward > outcome reward**: Chỉ reward kết quả cuối đúng/sai là không đủ. Cần supervise intermediate steps. Ví dụ: khi đánh giá AI viết báo cáo, không chỉ check kết quả cuối mà còn check từng step reasoning — có trích dẫn đúng nguồn không, có bỏ qua thông tin quan trọng không.
...
- Citation count: [EXTRACTION FAILED — Semantic Scholar not queried]
